...
var GeminiProvider = class {
  ...
  async streamText(prompt, systemInstruction, onChunk) {
    const model = this.ai.getGenerativeModel({
      model: this.modelName,
      systemInstruction
    });
...
var NvidiaNimProvider = class {
  ...
  constructor(apiKey, modelName = "meta/llama-3.1-70b-instruct") {
...
var NvidiaEmbeddingProvider = class {
  name = "nvidia-embedding";
  apiKey;
  modelName;
  baseUrl = "https://integrate.api.nvidia.com/v1/embeddings";
  constructor(apiKey, modelName = "nvidia/nv-embedqa-e5-v5") {
    this.apiKey = apiKey || process.env.NVIDIA_API_KEY || "";
...
// tests/verify_nvidia_provider.ts
...
async function runVerification() {
  ...
  const nimProvider = new NvidiaNimProvider(void 0, "meta/llama-3.1-70b-instruct");
  ...
  const jsonResponse = await nimProvider.generateJSON(
  ...
  const fallbackProvider = new NvidiaNimProvider(void 0, "nonexistent-overloaded-model");
